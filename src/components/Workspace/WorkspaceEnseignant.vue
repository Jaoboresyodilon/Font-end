...
function deleteEnseignant(data) {
  dialog.show(
  ...
  dialog.confirmation = () => {
    ...
    restApi
      .delete(`/api/enseignant/${data.id}`)
      .then(() => {
        alert.success("Enseignant supprimé avec succès !");
        reloadEnseignant();
      })
      .catch((error) => {
        alert.error("Erreur lors de la suppression de l'enseignant.");
        console.error(error);
      })
      .finally(() => {
